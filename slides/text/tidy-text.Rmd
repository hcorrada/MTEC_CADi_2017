...
```{r, dependson = "tidy_books", fig.width=6, fig.height=5, fig.cap="The most common words in Jane Austen's novels", eval=FALSE}
library(ggplot2)
...
```{r plotcount, dependson = "tidy_books", fig.width=9, fig.height=7, fig.cap="The most common words in Jane Austen's novels", echo=FALSE}
library(ggplot2)
...
```{r dependson = "frequency", fig.width=10, fig.height=5.5, fig.cap="Comparing the word frequencies of Jane Austen, the Brontë sisters, and H.G. Wells", eval=FALSE}
library(scales)
...
```{r dependson = "frequency", fig.width=10, fig.height=5.5, fig.cap="Comparing the word frequencies of Jane Austen, the Brontë sisters, and H.G. Wells", eval=TRUE, echo=FALSE}
library(scales)

# expect a warning about rows with missing values being removed
freq_plot <- ggplot(frequency, aes(x = proportion, y = `Jane Austen`, color = abs(`Jane Austen` - proportion))) +
  geom_abline(color = "gray40", lty = 2) +
  geom_jitter(alpha = 0.1, size = 2.5, width = 0.3, height = 0.3) +
  geom_text(aes(label = word), check_overlap = TRUE, vjust = 1.5) +
  scale_x_log10(labels = percent_format()) +
...
```{r, echo=FALSE, fig.width=10, fig.height=5.5, fig.cap="Comparing the word frequencies of Jane Austen, the Brontë sisters, and H.G. Wells"}
freq_plot
```

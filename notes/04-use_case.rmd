...
Now that we have our tools ready, let's start doing some analysis. We will use our illustrative analysis on movie ratings and gross revenue to learn our way around R.

We will learn:

- Basics of manipulating data with R expressions
- Ingesting data from files (using `readr`) or scraping from text (using `rvest`)
- Data manipulation and tidying using `dplyr` and `tidyr`
- Data visualization with `ggplot2`
- Basic clustering analysis and tidying of models with `broom`

## R principles

Before we begin, let's go over some principles of R as a data analysis environment. R is a computational environment for data analysis. It is designed around a _functional_ language, as opposed to _procedural_ languages like Java or C, that has desirable properties for the type of operations and workflows that are frequently performed in the course of analyzing datasets. In this exercise we will start learning some of those desirable properties while performing an analysis of a real dataset.
